...
jQuery.repeater = function () {
	...
	function repeat() {
		...
		let id = options.selecter + options.start;
		let html = $('#' + options.selecter + ' .clone:last').html().replace(/#no#/g, options.start).replace(/#id#/g, id);
		$('#' + options.selecter + ' .clone:last').clone(options.withDataAndEvents, options.deepWithDataAndEvents).attr('id', id).html(html).insertAfter($('#' + options.selecter + ' .clone:last')).show();
	}
